...
% * Quasi-Monte Carlo Sampling with SOBOL series
% * Nearest neighbour Shannon entropy 
...
dt_scatter=1; % Scatter plot
dt_diag=1; %diagnostic
...
%% Initialize particles - Sampling with Quasi Monte Carlo
sb=sobolset(2,'Skip',2);
xk=sb(1:Np,1)*L;% Uniformly distributed U(0,L)
vk=norminv(sb(1:Np,2))*1 + 0;  % Normally distributed N(0,1)
...
for t=0:dt:tmax
    ...
    for rksidx=1:length(rksd)
       ...
       %% Determine psi_k, all testfunctions,
        ...
        %% Phase space of scattered particles
        % Every particle carries the value of the initial distribution
        % along the characteristics.
        if (rksidx==1 && mod(t,dt_scatter)==0)
            
            scatter(xk,vk, 5,fk,'filled');
            xlabel('x');ylabel('v');
            shading interp; 
         ...
         if (rksidx==1 && mod(t,dt_diag)==0)
            ...
            weight=fk./gk;
            mass=mean(weight); 
            sh_entropy(floor(t/dt_diag)+1)=2*mean(log(dist(:,2)).*weight)...
                     + (double(eulergamma) ...
                       + log( pi^(2/2)/gamma(2/2+1))...
                       + log(Np) )*mass...
                     - mean(log(weight).*weight);             
         end
       
        
        %% Resample with radial basis Functions
...
%         nn=20;
%         net=newrb([xk(1:nn), vk(1:nn)]',fk(1:nn)' ,0.01, 0.02); % ,GOAL,SPREAD,MN,DF)
%         net
...
plot(0:dt_diag:tmax,sh_entropy)
xlabel('time'); grid on;
ylabel('entropy of f');
