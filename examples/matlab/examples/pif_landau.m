%% Particle in Fourier 2D for Vlasov-Poisson-Fokker-Planck
...
%% Initialize Particle in Fourier Poisson Solver
pif.kx=(2*pi)/L*(1:Nf);
...
if isempty(h)
    ...
    fprintf('This is very general, and will take some time!\n')
     ...
     for idx=1:Nf
         ...
         rhs_h(idx)=integral2...
             (@(x,v)  cos(2*pi/L*idx.*x).*h(x,v),...
                 0,L,-inf,inf,...
             'method','iterated',...
             'RelTol',1e-4,...
             'AbsTol',1e-5)+...
             1j*integral2...
             (@(x,v)  sin(2*pi/L*idx.*x).*h(x,v),...
                 0,L,-inf,inf,...
             'method','iterated',...
             'RelTol',1e-4,...
             'AbsTol',1e-5);
...
for t=0:dt:tmax
    ...
    for rksidx=1:length(rksd)
       ...
       %% Charge to grid, by spectral deposition
       % Evaluate all Fourier modes for all particles 
       psik=exp(1j*xk*pif.kx);
        
       
       %Fourier transform of charge density rho 
       rho_hat=(1/L)*mean( bsxfun(@times, psik, wk) );
       rho_hat=rho_hat.'+ (1/L)*rhs_cv;
       
       %Poisson solve
       phi_hat=1./(1j*pif.kx.').^2.*rho_hat;  
       % electric field
       E_hat=1./(1j*pif.kx.').*rho_hat;  

        
        %Diagnostic
        if (rksidx==1)
            kineticenergy(tstep)=0.5*mean(vk.^2.*fk./gk);
            fieldenergy(tstep)=(qm).^2*0.5*(E_hat'*E_hat*L*2);
        ...
        Ex=(real(psik)*real(E_hat)+imag(psik)*imag(E_hat))*2;
