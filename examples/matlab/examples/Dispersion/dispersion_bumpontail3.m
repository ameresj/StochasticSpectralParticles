...
fun=symfun(D(omega,0.2),omega);

close all;
figure;
% omegak=LuckStevensSearch(fun,20,0,1);

% omegak=McCuneSearch(fun,32,1024,0,2,1e-3);


digits(32);
% omegak=ContourSearch(fun,128,0,2,0.2);



omegak=NewtonSearch(fun,2048,20,0,2,1e-3);
 
omegak=Cz2Search( fun, 32, 256, z0, 1, 0.2, 1e-1 );
...
plot(unique(double(omegak)),'.')

%Estimate the numbers of zeroes in the box with 
contour_integrand=(diff(fun,omega)/fun/(1j*2*sym('pi')));
contour_integral=double(r*mean(contour_integrand(r*zk+z0)));
%contour_integral=abs(contour_integral);

if abs(contour_integral<=1)

double(mean((zk).^2./fun(r*zk+z0))./mean(zk./fun(r*zk+z0)))
else
    
z_new=z0+[(-1+1i),(1+1i),(1-1i),(-1-1i)]/sqrt(2)*(r/2);
end
...
%
radius=2; %
n=2e2;    % Number of unit roots, increase for precision
