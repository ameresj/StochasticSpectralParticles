...
%% Fokker-Planck
% Collision frequency
fokker_planck.theta=1e-2; % weak 1e-3, strong 0.1, none 0
    ...
    for rksidx=1:length(rksd)
       ...
       %% Determine psi_k, all testfunctions,
        ...
        if (fokker_planck.theta>0)
            ...
            etak=exp(-fp.theta./fp.sigma.^2./(1-exp(-2*fp.theta*dt)).*...
                ( (vk-fp.mu*(1-exp(-fp.theta*dt))-vk_old.*exp(-fp.theta*dt) ).^2 -...
                 (vk_old-fp.mu*(1-exp(-fp.theta*dt))- vk.*exp(-fp.theta*dt) ).^2 ) );
%             etak=1;
%  etak=fp.feq(vk)./fp.feq(vk_old);
             
            %Rescale weight
            gk=gk.*etak;
            fk=fk.*etak; vk_old=[]; etak=[];
        end
