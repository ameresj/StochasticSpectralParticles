...
%
testcase='landau'; % 'weibel','landau','weibels'
...
h0=@(x,v1,v2) exp(-v1.^2./sigma1^2/2 ).*...
    (delta*exp(-(v2-v01).^2/2/sigma2.^2) + ...
    (1-delta)*exp( - (v2-v02).^2/2/sigma2^2))/2/sigma1/sigma2/pi;
...
% Particle weight
% wk=fk./gk;
getweight=@(f,g,x,v1,v2) f./g;
getweight=@(f,g,x,v1,v2) (f-h0(x,v1,v2))./g;
...
B=zeros(length(kx),1);
E1cov=zeros(length(kx)); E2cov=E1cov; Bcov=E2cov;
%%
...
for tdx=1:Nt
    ...
    for splitdx=1:length(comp_gamma)
        ...
        hat=bsxfun(@times,wk.*vk2.*qk, conj(psik0));
        E2=E2  - mu0*(deltat/2)*mean(hat).'/L;
        E2cov=E2cov + cov(hat)*(mu0*(deltat/2)./L).^2;
        
        % Center of the Strang splitting, therefore 2*deltat/2
        %H_p1
        wk0=getweight(fk,gk,xk,vk1,vk2);
        xk=xk+ deltat*vk1;
        psik1=exp(1j*xk*kx.');
        wk1=getweight(fk,gk,xk,vk1,vk2);

        
        vk2=vk2 -  evF(psik1-psik0,(B./1j./kx) ).*qk./mk;

        
        hat=bsxfun(@times, wk1.*qk,conj(psik1))...
                       -bsxfun(@times, wk0.*qk,conj(psik0));
                   
        E1=E1 + mu0*mean( hat).'./1j./kx/L;
        E1cov=E1cov+ cov(hat)*diag(mu0./kx/L).^2;
        ...
        hat=bsxfun(@times,wk.*vk2.*qk, conj(psik1));
        E2=E2  - mu0*(deltat/2)*mean(hat).'/L;
        E2cov=E2cov + cov(hat)*(mu0*(deltat/2)./L).^2;
    ...
    Momentum(tdx,1)=mean(vk1.*wk);Momentum(tdx,2)=mean(vk2.*wk);
    Bpot(tdx)=(B'*B)*(L*2)/2;
end
runtime=toc(runtime);
