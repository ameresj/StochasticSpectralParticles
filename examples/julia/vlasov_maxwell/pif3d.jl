...
function eval_fourier_modes2{T<:AbstractFloat}(arg::T, N::Int,
    ...
    N0::Int=N+1
    mode[N0]=1.0
    const psin1::Complex{T}=cos(arg)+ im*sin(arg)
    psin::Complex{T}=one(Complex{T})
    @inbounds for kdx=1:N
       psin=psin*psin1
       @inbounds mode[N0+kdx]=psin
...
type pif3d{T}
  ...
  function pif3d{T}(Nx1,Nx2,Nx3,L1::T,L2::T,L3::T)
    self=new{T}(Nx1,Nx2,Nx3,L1,L2,L3,Nx1+1,Nx2*2+1,Nx3*2+1)
  ...
  function pif3d{T}(Nx1::Int,Nx2::Int,Nx3::Int,L::T)
    pif3d{T}(Nx1,Nx2,Nx3,L,L,L)
  end
end
...
function eval_basis{T}(self::pif3d, x1n::T,x2n::T,  x3n::T,
...
@inline @inbounds function eval_basis{T}(self::pif3d, x1n::T,x2n::T,  x3n::T,
  ...
  mode1::Complex{T}=cos(x1n*self.kx1) + im*sin(x1n*self.kx1)
  mode2::Complex{T}=cos(x2n*self.kx2) + im*sin(x2n*self.kx2)
  mode3::Complex{T}=cos(x3n*self.kx3) + im*sin(x3n*self.kx3)
...
@inline @inbounds function eval_basis2{T}(self::pif3d{T}, x1n::T,x2n::T,  x3n::T,
                            psin::Array{Complex{T},3})
...
function H1seminorm{T}(self::pif3d{T},F::Array{Complex{T},3} )
  K1,K2,K3=getK2(self)
  return 2.0*sum((abs(F.*K1).^2+abs(F.*K2).^2 +abs(F.*K2).^2)[:])*self.L1*self.L2*self.L3
end


function L2norm{T}(self::pif3d{T},F::Array{Complex{T},3}  )
  L2=sum(abs(F[:]).^2)*2.0- sum((abs(F[1,:,:]).^2)[:])
...
@inbounds function integrate_H_E{T}(self::pif3d{T}, E1::Array{Complex{T},3},
  ...
  integrate_H_E(self,x1n,x2n,x3n,v1n,v2n,v3n,qn,mn,E1,E2,E3,dt)

  integrate_Faraday(self,E1,E2,E3,B1,B2,B3,dt)
end


@inbounds function integrate_H_E{T}(self::pif3d{T}, x1n::Array{T,1},x2n::Array{T,1},
                        x3n::Array{T,1},
                        v1n::Array{T,1},v2n::Array{T,1}, v3n::Array{T,1},
                              qn::Array{T,1}, mn::Array{T,1},
                              E1::Array{Complex{T},3}, E2::Array{Complex{T},3},
                              E3::Array{Complex{T},3},
                                         dt::T)
