...
integer(8), PARAMETER :: Np=100000, Nf=64
...
do tdx = 1, Nt
    ...
    do rkdx = 1, size(rksd)
        ...
        ! Accumulate charge
        rho=0
        !$OMP PARALLEL DO reduction(+:rho) private(psin1,psin)
        do pdx=1,Np
            psin1=exp( DCMPLX(0.0, -xn(pdx)*kx0  ) )
            psin=DCMPLX(1.0,0.0)
            
            do kdx=1,Nf
                 psin=psin*psin1
                 rho(kdx)=rho(kdx)+( psin )*wn(pdx)
             end do
...
!             !$OMP SIMD
!             do kdx=1,Nf
!                 rho(kdx)=rho(kdx)+rhon(kdx)*wn(pdx)
!             end do
        end do
        !$END OMP PARALLEL DO
        rho=rho/L/(1.0*Np)
